	...
	if ( $query->have_posts() ) : 
		while ( $query->have_posts() ) : 
			...
			if(!empty($majc_settings)) {
				...
				// Main Settings
				$layout_type = 'slidein'; 
				$ajax_enabled = isset($majc_settings['enable_ajax_atc']) ? true : null;
				...
				$header_show_total_items = isset($header['show_cart_item_count']) ? $header['show_cart_item_count'] : 0;
				...
				<div class="majc-cart-popup <?php echo !empty($show_animation) && !empty($hide_animation) ? 'majc-cartpop-animation-enabled animate__animated' : ''; ?>" 
					...
					<?php } ?>
				>
					<div class="majc-cart-popup-inner" <?php if( $content_bg_type == 'choose_color' ) { echo 'style="background-color:'.$content_bg_color.';"'; } ?> <?php if( $content_bg_type == 'custom_image_bg' ) { echo 'style="background: url('.$content_bg_image.';"'; } ?>>
					...
					<div class="majc-body">
						...
						<div class="majc-cart-action-btn-wrap">
					        <h4><?php echo esc_html($shipping_text); ?></h4>
